 ...
 * @module base-crud-listallfields
 * @since 1.0.0
 * @author Z-Kali
 * @tags [crud],[list],[pagination],[scope],[include],[allfields]
 * @description 分页列表查询（返回 listFields ∪ detailFields 全部字段），支持关联查询和 Scope
 * @path src/repo/base/crud/listallfields.ts
...
import { allFields, QueryOptionsBuilder } from "./helpers";

/**
 * @function listAllFieldsAsync
 * @description 分页列表查询（返回 listFields ∪ detailFields 全部字段），支持关联查询和 Scope
 ...
 */
export async function listAllFieldsAsync<
  M extends Model,
  A extends object
>(
  ...
  const { defaultOrder = [["createdAt" as keyof A & string, "DESC"]], searchFields = [] } = config;
  const { page = 1, limit = 20, search, filters, order } = q;

  // 合并 listFields 与 detailFields 获取全部字段
